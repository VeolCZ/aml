...
def train_vit() -> None:
    ...
    train_indices, _, _, _ = train_test_split(
        np.arange(len(train_dataset)), all_labels, test_size=TEST_SIZE, stratify=all_labels, random_state=SEED
    )
...
def eval_vit() -> None:
    ...
    _, test_indices, _, _ = train_test_split(
        np.arange(len(eval_dataset)), all_labels, test_size=TEST_SIZE, stratify=all_labels, random_state=SEED
    )
